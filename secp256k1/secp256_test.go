...
func TestMultiSetInit(t *testing.T) {

	set, err := MultiSetInit()
	assert.Nil(t, err, "should be nil")
	assert.NotNil(t, set, "should be nil")
}

func TestMultiSetInitAddFinalize(t *testing.T) {

	set, err := MultiSetInit()

	bytes, err := hex.DecodeString("d9f01d93b215ac701c7f9a56c8d5d1bdf2e0498047da81d3bf6fc23a2ee3bbb5")
	assert.Nil(t, err, "should be nil")

	err = MultiSetAdd(set, bytes)
	assert.Nil(t, err, "should be nil")

	hash, err := MultiSetFinalize(set)
	assert.Nil(t, err, "should be nil")

	assert.NotNil(t, hash, "should be nil")
	t.Log("TestMultiAddFinalizeInit hash: ", hash)
}


func TestMultiSetInitAddRemoveFinalize(t *testing.T) {
